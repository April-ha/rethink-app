...
interface CountryConfigDAO {
    ...
    @Query("SELECT cc FROM CountryConfig WHERE catchAll = 1 AND isEnabled = 1")
    suspend fun getCatchAllCountryCodes(): List<String>

    @Query("SELECT cc FROM CountryConfig WHERE lockdown = 1 AND isEnabled = 1")
    suspend fun getLockdownCountryCodes(): List<String>

    @Query("SELECT cc FROM CountryConfig WHERE mobileOnly = 1 AND isEnabled = 1")
    suspend fun getMobileOnlyCountryCodes(): List<String>

    @Query("SELECT cc FROM CountryConfig WHERE ssidBased = 1 AND isEnabled = 1")
    suspend fun getSsidBasedCountryCodes(): List<String>

    @Query("SELECT * FROM CountryConfig WHERE isEnabled = 1 ORDER BY priority DESC, lastModified DESC")
    ...
    @Query("UPDATE CountryConfig SET isEnabled = :value, lastModified = :timestamp WHERE cc = :cc")
    ...
    @Query("SELECT COUNT(*) FROM CountryConfig WHERE isEnabled = 1")
    suspend fun getEnabledCount(): Int

    @Query("SELECT COUNT(*) FROM CountryConfig WHERE catchAll = 1 AND isEnabled = 1")
    suspend fun getCatchAllCount(): Int

    @Query("SELECT COUNT(*) FROM CountryConfig WHERE lockdown = 1 AND isEnabled = 1")
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM CountryConfig WHERE cc = :cc AND isEnabled = 1)")
